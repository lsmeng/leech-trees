...
Secondary claims (Section 6 of the paper): M(11) = 60 and M(12) = 77, in each case with exactly two minimal trees; no modular Leech tree of order 9 or 11; modular Leech trees of order 5 do exist; exactly six 6-leaf leaf-Leech trees, five with an irreducible vertex, answering Question 3.1 of Ozen–Wang–Yalman (2016) affirmatively.
...
Repository root: `~/Documents/claude/projects/leech-trees` (git; record the exact commit you audit). Python environment: `.venv` (ortools, networkx). Build: `scripts/build.sh` (clang++ -O3 -std=c++17).
...
This work was carried out by AI agents, principally Claude Code and OpenAI Codex, under the direction of a human author who is a seismologist, not a mathematician, over roughly 24 hours. The verification chain — two checkers, an adversarial referee pass, a clean-room re-implementation, cross-architecture replication — was built precisely because of that. Two errors were already caught internally by that chain and are documented (`docs/referee-forest.md`, the Lemma 5(b) correction in `docs/theory-notes.md`). Treat this history as a reason for *more* scrutiny of the mathematical statements and the literature claims, not less: the code has been attacked hard, the scholarship less so.

---

## 5A. ARFT checks: did the research process contradict the paper?

Use the failure patterns in Fei et al., [*How Do Agents Fail on AutoResearch*](https://arxiv.org/html/2608.14905v2) (arXiv:2608.14905v2), as a compact checklist, not as an authority about this result or about model capability. Their artifact-aware judge examined code, logs, reports and data, and required a verifiable anchor for every issue. That design matters here. In their 800 trajectories the three non-engineering root-cause pillars account for 92.1% of 12,712 HITs, while Engineering Robustness accounts for 7.9%; the most frequent patterns include F.4 (660/800), E.2 (625/800), D.4 (620/800), F.2 (502 HITs) and D.7 (486 HITs). These frequencies are prompts for where to look, not prior probabilities that this project is wrong.

Apply these translations to the complete artifact package, not just the manuscript:

- **F.2 / F.4 — a flaw was diagnosed but never gated or repaired.** Build a disposition table for every BUG, UNCLEAR item, robustness nit and correction in `docs/referee-report.md` and `docs/referee-forest.md`: where is it fixed in the final source, tests and manuscript, or why is it non-load-bearing? The required calibration case is the erroneous Golomb-ruler version of Lemma 5(b): trace the counterexample and withdrawal in `docs/referee-report.md` to the corrected weak-Sidon statement in `docs/theory-notes.md`, its machine checks, and the wording actually shipped in `paper/main.tex`. A review paragraph saying “fixed” is not remediation.
- **D.7 — adverse evidence was logged and then neutralised rhetorically.** At the current ledger snapshot the per-topology corroboration reports 112,082/122,344 survivor topologies UNSAT, 1,169 re-run cases still UNKNOWN at the one-hour cap, and 0 SAT; use the live `README.md` numbers if they have advanced. Confirm that neither the UNKNOWN nor the not-yet-redecided cases are counted as UNSAT or used to prove the theorem. Likewise inspect every `capped=1` line in `theory-lab/exp-families/*.out`: `found=0, capped=1` is an incomplete probe, never a nonexistence result. Search for every place these results feed the abstract, theorems, discussion and captions.
- **E.2 / E.3 — selective narrative or omitted limits.** Compare the abstract and conclusion against the weakest entries in `README.md`, `variants/RESULTS.md`, `theory-lab/WORKFLOW-RESULTS-RAW.json` and the run ledger. Hunt especially for capped searches, the single-implementation modular order-11 result, unresolved access to Luo–Yu (2024) and Leach–Walsh (2011), and experimental finite-family evidence presented as a theorem. Report unfavourable facts that appear in the repository but disappear from the paper.
- **D.4 / C.3 — the method, implementation and conclusion decide different propositions.** Trace each theorem to the exact production code path and terminal condition. Check the forcing-search terminal test, vertex and distance caps, integer widths, and the meanings of `DONE`, `UNKNOWN`, `capped`, `found` and `nsol`. Distinguish the complete forced-forest proof from the incomplete per-topology corroboration. For the neighbouring problems, check that each engine's target set and equivalence convention match the definition printed in Section 6.
- **B.1 / B.4 / B.5 — invented, shallow or decorrelated scholarship.** Resolve every `paper/refs.bib` entry to the stated authors, title, year, venue and pages, then verify that the accessible source supports the sentence citing it. Do not silently convert “not obtained” into “checked”: Luo–Yu (2024) and Leach–Walsh (2011) remain explicit trouble spots. As a calibration example, an earlier scout attributed arXiv:2210.03262 to “Rowley”; the [arXiv record](https://arxiv.org/abs/2210.03262) lists Yuan Chang, Jesús A. De Loera and William J. Wesley. Search beyond the bibliography for similarly confident names or priority claims without a retrieved source.
- **C.1 — circular validation masquerading as independence.** Determine whether `checker_a.py` and `checker_b.py` differ in algorithm and assumptions rather than filenames; whether the two topology enumerations can fail together; and whether `cleanroom/cr_forest.c` was genuinely produced without access to `forest_search.cpp`. Inspect git history and provenance claims, and write an independent micro-oracle or hand calculation where feasible. Agreement between descendants of the same logic is not independent evidence.
- **X.6 — right answer for the wrong reason.** The level-17 count is zero. Establish why: rule out an unintended node/time cap, `UNKNOWN` treated as done, a vertex-count off-by-one, overflow, an ignored command-line flag, or a shard that never ran. Positive controls and planted instances show that witnesses can survive, but they do not by themselves prove that this production run exhausted the intended state space; connect source identifiers, invocation, shard records and summed histograms into one causal chain.

Limits of this checklist must appear in your audit report. Six of the eight evaluated harness–model combinations use Claude Code; the Codex and Gemini CLI rows use only `gpt-5-mini` and `gemini-3.5-flash`, respectively, so the paper's model-versus-scaffold attribution is not well controlled. Its artifact-aware judge has 80.7% pattern recall against the 50-trajectory human sample (versus 63.5% for transcript-only judging), so it still misses about 19% of labelled patterns; agreement is reported only in aggregate, and the Cognitive Depth & Adaptability pillar—27.6% of HITs—is explicitly lower-confidence. Treat ARFT as vocabulary and procedure, not as an established measurement of this auditor or this project.
...
**Evidence rule (mandatory).** Every finding, including a statement that a critical item was checked and passed, must give at least one verifiable anchor: a file name plus line number, a code identifier, a log line or shard record, an exact numeric value, or a reproduction command with its output. Unsupported assertions in either direction — including “I checked this and it is fine” — do not count. Identify the exact commit audited and distinguish evidence you inspected from evidence you independently reproduced.
