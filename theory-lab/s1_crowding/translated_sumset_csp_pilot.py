...
[1, delta].  Optional flags add occupied owners and exact low-low distances;
general low-high constraints remain omitted in this pilot.
"""
...
def run(
    ...
    for s in range(1, s_max + 1):
        ...
        def dfs(i: int, used_now: set[int]):
            ...
            nonlocal occupied_prune_rejects, hole_out_of_range_rejects
            nonlocal low_low_prune_rejects
            nodes += 1
            if nodes > node_limit:
                complete = False
                return
            ...
            for l in range(lo, B):
                ...
                if include_known_occupied:
                    # The low-low root/hole pair has distance s+l.  It is a
                    # genuine owner, so an out-of-range value is a sound
                    # rejection rather than something to silently omit.
                    hole = s + l
                    if hole < 1 or hole > delta:
                        hole_out_of_range_rejects += 1
                        L[i] = None
                        continue
    ...
    result = {
        ...
        "hole_out_of_range_rejects": hole_out_of_range_rejects,
