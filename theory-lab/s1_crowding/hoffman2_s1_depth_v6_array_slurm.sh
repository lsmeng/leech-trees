...
cd "$ROOT"
mkdir -p logs depthv6_${TAG}
K=${SLURM_ARRAY_TASK_ID}
./abstract_class_search6 --m $M --shapes-file $SHAPES --solve-depths --rho-check 6,9,12 --max-witness 50 --split $LVL $K $NSPLIT \
  > depthv6_${TAG}/split_${K}_of_${NSPLIT}.txt.tmp 2>&1 && mv depthv6_${TAG}/split_${K}_of_${NSPLIT}.txt.tmp depthv6_${TAG}/split_${K}_of_${NSPLIT}.txt
