...
cd "$ROOT"
mkdir -p logs depth_${TAG}
K=${SLURM_ARRAY_TASK_ID}
./abstract_class_search_v4 --m $M --shapes-file $SHAPES --solve-depths --max-witness 50 --split $LVL $K $NSPLIT \
  > depth_${TAG}/split_${K}_of_${NSPLIT}.txt.tmp 2>&1 && mv depth_${TAG}/split_${K}_of_${NSPLIT}.txt.tmp depth_${TAG}/split_${K}_of_${NSPLIT}.txt
