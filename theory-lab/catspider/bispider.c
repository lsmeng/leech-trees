 ...
 *
 * usage: bispider n [LR|LL|both] [T-lo T-hi] [--r-lo R --r-hi R] [--fast]
 *        (T range restricts the outer anchor loop of BOTH cases for sharding:
 *        LR shards on TL, LL on T1; --fast disables the diagnostic full
 *        recomputation and is for window probes, never certificate runs;
 *        --r-lo/--r-hi filter LR anchors by r=Q-(T_R-T_L)=N-2*T_R)
...
static int max_missing_offset=0, max_noncentre_marks=0;
...
static void dfs(int v){
    ...
    while(v>=1 && bit(v)) v--;
    int offset=N-v;
    if(offset>max_missing_offset) max_missing_offset=offset;
    if(nmarks>max_noncentre_marks) max_noncentre_marks=nmarks;
    if(v==0){
...
int main(int argc,char**argv){
    if(argc<2){ fprintf(stderr,"usage: %s n [LR|LL|both] [lo hi] [--r-lo R --r-hi R] [--fast]\n",argv[0]); return 1; }
    n=atoi(argv[1]); N=n*(n-1)/2;
    const char* mode = argc>2?argv[2]:"both";
    int lo=1, hi=N, rlo=-N, rhi=N, rfilter=0;
    int numeric_args=0;
    for(int i=3;i<argc;i++){
        if(!strcmp(argv[i],"--fast")){ paranoid=0; continue; }
        if(!strcmp(argv[i],"--r-lo")||!strcmp(argv[i],"--r-hi")){
            int islo=!strcmp(argv[i],"--r-lo");
            if(++i>=argc){ fprintf(stderr,"%s requires an integer\n",argv[i-1]); return 1; }
            if(islo) rlo=atoi(argv[i]); else rhi=atoi(argv[i]);
            rfilter=1; continue;
        }
        if(numeric_args==0) lo=atoi(argv[i]);
        else if(numeric_args==1) hi=atoi(argv[i]);
        else { fprintf(stderr,"too many range arguments\n"); return 1; }
        numeric_args++;
    }
    if(numeric_args==1){ fprintf(stderr,"range requires both lo and hi\n"); return 1; }
    if(rlo>rhi){ fprintf(stderr,"invalid r range\n"); return 1; }
    if(rfilter&&strcmp(mode,"LR")){ fprintf(stderr,"r filtering is available only in LR mode\n"); return 1; }
    ...
    printf("{\"n\":%d,\"N\":%d,\"mode\":\"%s\",\"range\":[%d,%d],\"r_range\":[%d,%d],\"branches\":%llu,"
           "\"nodes\":%llu,\"nsol\":%llu,\"deepest_missing_offset\":%d,"
           "\"max_noncentre_marks\":%d,\"paranoid\":%d,\"secs\":%.2f,\"status\":\"DONE\"}\n",
        n,N,mode,lo,hi,rlo,rhi,(unsigned long long)branches,(unsigned long long)nodes,
        (unsigned long long)nsol,max_missing_offset,max_noncentre_marks,paranoid,secs);
    return 0;
}
