#!/usr/bin/env python3
"""
Independent exact-10 full-low-owner component-root checker/replay.
...
class Exact10Checker:
    def __init__(self, s: int, shape: str, d1_shard=None):
        ...
        self.roots = []
        self.local_color_mask = [0] * (Y_MAX + 1)
        ...
        self.dfs_calls = 0
        self.full_high_coloring_calls = 0
    ...
    def setup_parameter(self, d1: int, d2: int) -> bool:
        ...
        # Immutable-background-only necessary filter.  Dynamic collisions
        # remain in try_add_root(), so this cannot remove a genuine root.
        self.local_color_mask = [0] * (Y_MAX + 1)
        immutable = set(self.owner_values)
        for y in range(Y_MIN, Y_MAX + 1):
            mask = 0
            for color in range(3):
                if not self.local_attachment_legal(y, color):
                    continue
                du1h = self.low_high_distance(1, y, color)
                du2h = self.low_high_distance(2, y, color)
                if du1h == du2h:
                    continue
                if not self.pair_value_allowed(du1h):
                    continue
                if not self.pair_value_allowed(du2h):
                    continue
                if du1h in immutable or du2h in immutable:
                    continue
                mask |= 1 << color
            self.local_color_mask[y] = mask
    ...
    def low_high_distance(self, low_vertex: int, y: int, color: int) -> int:
        hdepth = self.B + y
        if self.shape == "star":
            if low_vertex == 1:
                return hdepth - self.d1 if color == 1 else hdepth + self.d1
            if low_vertex == 2:
                return hdepth - self.d2 if color == 2 else hdepth + self.d2
            raise ValueError(f"bad low_vertex {low_vertex}")
        if low_vertex == 1:
            return hdepth + self.d1 if color == 0 else hdepth - self.d1
        if low_vertex == 2:
            if color == 0:
                return hdepth + self.d2
            if color == 1:
                return hdepth + self.d2 - 2 * self.d1
            if color == 2:
                return hdepth - self.d2
            raise ValueError(f"bad color {color}")
        raise ValueError(f"bad low_vertex {low_vertex}")
    ...
    def local_attachment_legal(self, y: int, color: int) -> bool:
        ...
        if not (0 < att < self.s):
            return False
        if color == 0:
            return att == self.B + y and att in self.owner_values
        return self.pair_value_allowed(att) and att not in self.owner_values
    ...
    def try_add_root(self, y: int, color: int):
        ...
        if not (self.local_color_mask[y] & (1 << color)):
            return None
        if not self.local_attachment_legal(y, color):
            return None

        hdepth = self.B + y
        attachment = self.attachment_value(y, color)
        if hdepth not in self.owner_values:
            return None

        pending = set()

        def propose(value: int) -> bool:
            if not self.pair_value_allowed(value):
                return False
            if value in self.owner_values or value in pending:
                return False
            pending.add(value)
            return True
        ...
        du1h = self.low_high_distance(1, y, color)
        du2h = self.low_high_distance(2, y, color)
        if not propose(du1h) or not propose(du2h):
            return None

        if color == 0:
            if attachment != hdepth:
                return None
        elif color == 1:
            if attachment != du1h:
                return None
        ...
        for old_y, old_color in self.roots:
            dist = self.actual_root_distance(y, color, old_y, old_color)
            if not propose(dist):
                return None
    ...
    # ------------------------------------------------------------------
    # Independent full-21 low-high owner extension
    # ------------------------------------------------------------------

    def has_full_high_coloring_rec(self, y: int, selected_y: set) -> bool:
        self.full_high_coloring_calls += 1
        if y > Y_MAX:
            return True
        if y in selected_y:
            return self.has_full_high_coloring_rec(y + 1, selected_y)

        # Deliberately different color order from the C++ implementation.
        for color in (1, 2, 0):
            du1h = self.low_high_distance(1, y, color)
            du2h = self.low_high_distance(2, y, color)
            if du1h == du2h:
            ...
            if not self.pair_value_allowed(du2h):
                continue
            if du1h in self.owner_values or du2h in self.owner_values:
                continue

            self.owner_values.add(du1h)
            self.owner_values.add(du2h)
            ok = self.has_full_high_coloring_rec(y + 1, selected_y)
            self.owner_values.remove(du2h)
            self.owner_values.remove(du1h)
            if ok:
                return True
        return False

    def has_full_high_coloring(self) -> bool:
        return self.has_full_high_coloring_rec(
            Y_MIN,
            {y for y, _ in self.roots},
        )
    ...
    def dfs(self, next_y: int, need: int):
        ...
        if need == 0:
            if self.has_full_high_coloring():
        ...
        if sum(
            1
            for y in range(next_y, Y_MAX + 1)
            if self.local_color_mask[y]
        ) < need:
            return
    ...
    def result(self):
        ...
        result = {
            "status": "INDEPENDENT_EXACT10_FULL_21_COLOR_SUBSYSTEM_EXHAUSTED",
            "s": self.s,
            "B": self.B,
            "shape": self.shape,
            ...
            "full_high_coloring_calls": self.full_high_coloring_calls,
...
def compare_expected(actual, expected):
    ...
    expected_roots = sorted(
        signature.split("|C=", 1)[0]
        for signature in expected.get("first_examples", [])
    )
    if sorted(actual["first_examples"]) != expected_roots:
        raise AssertionError(
            "canonical root signature mismatch:\n"
            f"checker={sorted(actual['first_examples'])!r}\n"
