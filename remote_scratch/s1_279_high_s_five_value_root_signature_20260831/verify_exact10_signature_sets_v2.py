#!/usr/bin/env python3
"""Verify exact-10 signature sets with order-independent instrumentation.

The recursive extension-node count is retained as a diagnostic because both
searches short-circuit on the first witness and deliberately use different
color orders.  The wrapper invocation count is the comparable invariant.
"""
...
def main() -> None:
    ...
    primary_raw = [str(value) for value in primary["first_examples"]]
    primary_normalized = [normalize_primary(value) for value in primary_raw]
    replay_signatures = [str(value) for value in replay["canonical_root_signatures"]]
    ...
    semantic_checks = {
        ...
        "dfs_calls": primary["dfs_calls"] == replay["dfs_calls"],
        "extension_invocations": (
            primary["full_high_coloring_invocations"]
            == replay["extension_invocations"]
        ),
    ...
    diagnostic = {
        "primary_recursive_extension_calls": int(primary["full_high_coloring_calls"]),
        "replay_recursive_extension_calls": int(replay["extension_calls"]),
        "recursive_extension_call_delta": (
            int(replay["extension_calls"])
            - int(primary["full_high_coloring_calls"])
        ),
        "recursive_extension_calls_equal": (
            primary["full_high_coloring_calls"] == replay["extension_calls"]
        ),
        "noncomparable_reason": (
            "Both implementations stop at the first full-21 witness, but primary "
            "tries colors 0,1,2 and replay tries 1,2,0."
        ),
    }
    ...
    status = (
        "VERIFIED_EXACT10_SIGNATURE_SET_MATCH"
    ...
    certificate = {
        ...
        "extension_invocations": int(primary["full_high_coloring_invocations"]),
