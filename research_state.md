...
- **2026-09-04（Fable）：两份独立审稿意见的核查与处理。**
  ...
  **我与审稿方的一处分歧**：B 第 8 条称逐层增长因子为 8.3–9.4，我算得 7.1/7.6/8.3/8.4，
  ...
  暂已改为准确的将来时表述）。
- **2026-09-04（Fable）：M(13) 上界由 105 改为 110，并给出可核验见证；105 因见证遗失而撤回。**
  论文原称 `M(13) ≤ 105` 依赖一棵"最大距离 79 的 12 顶点见证"，`variants/RESULTS.md` 只记了
  "`... 4-12:62` appended to the D=79 tree"，**该树既不在工作区也不在 git 历史中**。
  两轮集群搜索均失败：先 128 个分片（错误模式——`--shard` 配 `--maxsol 1` 时无解的分片必须
  穷尽自己那一片，而 D=77 的穷尽是 1.02e12 节点），后 4 个不分片 `--edge-first` 任务
  （D=79..82，各 8 小时全部 TIMEOUT 且零输出，疑似缓冲未刷新即被 SIGKILL）。合计约 77 CPU 小时无所获。
  另试从两棵 11 顶点最小树（最大 60）加两片叶子，最好仅得 123，劣于一片叶子的 110。
  **决定：停止搜索，取可核验的 110。** 该树为已印出的 D=78 见证加边 `9--12:69`，
...
补记：v1.2 首次打标签时 §11 仍指向 v1.1（论文自指版本不一致）。已改为指向 v1.2 并说明 v1.0/v1.1 保持不动，
随后**重新指向 v1.2 标签与 Release**（该标签当时仅存在数分钟、无外部引用，故移动优于再开 v1.3；此处如实记录）。

## 2026-09-07 v1.2 归档发布；arXiv 暂不动
- GitHub：标签 **v1.2-deposit-2026-09-07**（commit 9883038）+ Release 已发布；v1.0、v1.1 原地不动。
  压缩包 10,138,136 B，MD5 69256b75558405c071c747cc68a1cb19；已核对包内 `paper/main.tex` 自指 v1.2（自洽）。
- Zenodo 两条记录均出新版本（concept DOI 不变，始终指向最新）：
  * 预印本 **10.5281/zenodo.22652903**（concept 10.5281/zenodo.22573640），文件 Meng-Leech-final-2026-09-07.pdf；
  * 代码证书 **10.5281/zenodo.22652936**（concept 10.5281/zenodo.22334398），文件 v1.2 源码包 + forest19_20 分片包。
  两条记录的说明里都写明了本次修订的原因（Ghodsi 段落遗失并恢复），不掩饰。
- **arXiv 8042742 暂不修改**：状态仍为 on hold，账户页对该条只提供 resume 与"联系审核员删除"，**没有 unsubmit**；
...
## 2026-09-07 Zenodo 已同步新题目（两条记录各出新版本）
- 预印本：**10.5281/zenodo.22653637**（concept 10.5281/zenodo.22573640），Preprint，
  version "As submitted to AJC, 7 September 2026"，文件 Meng-Leech-final-2026-09-07.pdf（MD5 80575c08…，与本地一致）。
- 代码证书：**10.5281/zenodo.22654236**（concept 10.5281/zenodo.22334398），Software，version v1.3，
  文件 leech-trees-v1.3-deposit-2026-09-07.tar.gz（MD5 a5e35742…）+ forest19_20_shards.tar.gz。
  两条记录的标题均已改为新题目，说明里写明了改题原因（25 阶界本身未变，摘要与 §8 完整保留）。
- **已知的小不一致，留待下次修订**：论文 §11 引的是版本 DOI 10.5281/zenodo.22652936（内含 v1.2 压缩包），
  而正文说标签是 v1.3。该 DOI 永久有效且代码/证书与 v1.3 完全相同（只有论文文本不同），实用上无碍。
  **下次修订应改引 concept DOI 10.5281/zenodo.22334398**，它永远指向最新版本，可一劳永逸解决这种自指追赶。
